...
import sig.eu.demo.persistency.RoleRepository;
import sig.eu.demo.persistency.UserRepository;
import sig.eu.demo.persistency.VerificationTokenRepository;
...
public class UserService implements IUserService {
    ...
    public User findUserByEmail(final String email) {
        ...
        return (((Optional)user).isPresent()) ? user.get() : null;
    }
    ...
    public String generateQRUrl(User user) throws UnsupportedEncodingException {
        return QR_PREFIX + URLEncoder.encode(
            String.format("otpauth://totp/%s:%s?secret=%s&issuer=%s", APP_NAME, user.getEmail(), user.getSecret(),
                APP_NAME), "UTF-8");
    }
